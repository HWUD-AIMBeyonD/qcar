...
class StanleyControllerServer : public rclcpp::Node
{
public:
  ...
  StanleyControllerServer() : Node("stanley_controller_server")
  {
    ...
    controller_ = std::make_shared<stanley_controller::StanleyController>();
    ...
    action_server_ = rclcpp_action::create_server<Action>(
      ...
      "FollowPath",
      std::bind(&StanleyControllerServer::handle_goal, this, _1, _2),
      std::bind(&StanleyControllerServer::handle_cancel, this, _1),
      std::bind(&StanleyControllerServer::handle_accepted, this, _1)
    );
  ...
  void setup() 
  {
      RCLCPP_INFO(get_logger(), "Configuring Stanley Controller...");
      
      rclcpp::Node::WeakPtr weak_node = this->weak_from_this();
      
      // Pass the Action Server name "FollowPath" to look up params under FollowPath
      controller_->configure(weak_node, "FollowPath", tf_buffer_, costmap_ros_);
      controller_->activate();
      
      RCLCPP_INFO(get_logger(), "Stanley Controller Server Ready!");
  }

  ~StanleyControllerServer() {
    if(controller_) {
...
private:
  ...
  {
    std::thread{std::bind(&StanleyControllerServer::execute, this, _1), goal_handle}.detach();
  }
  ...
  std::shared_ptr<stanley_controller::StanleyController> controller_;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<StanleyControllerServer>();
  node->setup();
