...
class VectorPursuitControllerServer : public rclcpp::Node
{
public:
  ...
  VectorPursuitControllerServer() : Node("vector_pursuit_controller_server")
  {
    ...
    // 2. Setup Costmap (Uses node name to find params)
    ...
    controller_ = std::make_shared<vector_pursuit_controller::VectorPursuitController>();
    ...
    action_server_ = rclcpp_action::create_server<Action>(
      ...
      "FollowPath",
      std::bind(&VectorPursuitControllerServer::handle_goal, this, _1, _2),
      std::bind(&VectorPursuitControllerServer::handle_cancel, this, _1),
      std::bind(&VectorPursuitControllerServer::handle_accepted, this, _1)
    );
  ...
  void setup() 
  {
      RCLCPP_INFO(get_logger(), "Configuring Vector Pursuit Controller...");
      
      rclcpp::Node::WeakPtr weak_node = this->weak_from_this();
      
      // Pass "FollowPath" to look up params under the FollowPath namespace
      controller_->configure(weak_node, "FollowPath", tf_buffer_, costmap_ros_);
      controller_->activate();
      
      RCLCPP_INFO(get_logger(), "Vector Pursuit Controller Server Ready!");
  }

  ~VectorPursuitControllerServer() {
    if(controller_) {
...
private:
  ...
  {
    std::thread{std::bind(&VectorPursuitControllerServer::execute, this, _1), goal_handle}.detach();
  }
  ...
  void execute(const std::shared_ptr<GoalHandle> goal_handle)
  {
    ...
    // Prepare Standard Path (Dashing fix for header stamps)
    ...
    for(const auto & incoming_pose : goal->path.poses) {
        ...
        // CRITICAL FIX for Dashing build error
  ...
  std::shared_ptr<vector_pursuit_controller::VectorPursuitController> controller_;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<VectorPursuitControllerServer>();
  node->setup();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
